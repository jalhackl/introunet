...
def get_vcf_bed_folder(folder, ignore_zero_introgression = True, real_data = False):
    """
    Description:
                ...
                if real_data == False:
                    ...
                    if ignore_zero_introgression == True:
                        if os.stat(os.path.join(folder, dirr, bed_file)).st_size == 0:
                            ...
                            continue
                #set to None in case of real data, i.e. if no bed_file with the true tracts is available
                else:
                    bed_file = None
...
def format_from_vcf_df_true_data(vcf_file, return_all=True, upsample=112/2, uniform_upsampling=True, ploidy=2, ref_size=50, target_size=50, select_populations=True, merge_haplotypes=False, compute_scaled_positions=False, fragment_length=50000, add_same_number=False):
    """
    Description:
        loading formatting of the input vcf and bed file 
        lists containing the haplotype arrays, the introgression arrays and the positions are returned
    ...
    if select_populations == True:
        ...
        if return_all == True:
            return pop_df_ref, pop_df_target, None, None, newsamples_ref, newsamples_target, newpos
        
        
    return intro_array2, pop_df, None
