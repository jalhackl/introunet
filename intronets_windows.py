...
def get_matrices_multiproc(files, polymorphisms=128, stepsize=16, random_reg=False, random_el=1, remove_samples_wo_introgression=False, only_no_introgression = False, only_first=False, return_also_pos = True):
    """
    Description:
...
def get_matrices_multiproc_padding(files, windowsize=50000, max_polymorphisms=256, stepsize=16, random_reg=False, random_el=1, remove_samples_wo_introgression=False, only_first=False, return_also_pos = True):
    """
    Description:
    ...
    if random_reg == False:
        ...
        startpos = 0
        while (startpos + windowsize) <=  (pop_df_ref_genotype.shape[1]):
            
            endpos = startpos + windowsize
            #wir muessen den ersten kleineren entry finden...
            pos_indexes = np.where((newpos < startpos) | (newpos > endpos))[0]
            if len(pos_indexes) < max_polymorphisms:
                print("PROBLEM: the number of polymorphisms in the window is greater than the value indicated by max_polymorphisms!")

            #newpos_win = newpos[startpos:endpos]
            
            pop_df_ref_genotype_win = pop_df_ref_genotype[:,pos_indexes]
            pop_df_target_genotype_win = pop_df_target_genotype[:,pos_indexes]
            intro_df_ref_genotype_win = intro_df_ref_genotype[:,pos_indexes]
            intro_df_target_genotype_win = intro_df_target_genotype[:,pos_indexes]
            positions = [startpos, endpos]

            newpos_win = newpos[pos_indexes]
        ...
        for ir in range(random_el):
            ...
            startpos = np.random.randint(0, geno_length - windowsize)
            ...
            if max_polymorphisms > geno_length:
                print("the haplotype/chromosome is too short!")
                continue
