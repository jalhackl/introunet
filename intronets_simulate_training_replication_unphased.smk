...
rule create_dataframes_prediction:
    ...
    run:
        ...
        for i in range(int(nrep_folder)):
            ...
            if not no_window:
                all_entries = process_vcf_df_windowed_multiproc(new_output_dirs, polymorphisms=polymorphisms, stepsize=stepsize, random_reg=random_restrict, random_el=random_el, ignore_zero_introgression=remove_samples_wo_introgression, only_first=only_first, list_of_folders=True, merge_haplotypes_for_unphased_prediction=True)
            else:
