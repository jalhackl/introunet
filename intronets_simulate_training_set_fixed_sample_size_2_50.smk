...
hdf_filename = "100k_random_wo_2_50.h5"
new_hdf_file = hdf_filename
output_dir = "100k_random_wo_2_50"
    ...
    run:
        ...
        while (created_samples < desired_samples):
            ...
            for ipoly, polymorphisms in enumerate(polymorphisms_list):
                ...
                if no_window == False:
                    all_entries = process_vcf_df_windowed_multiproc(new_output_dirs, polymorphisms=polymorphisms, stepsize=stepsize, random_reg=random_restrict, random_el=random_el, ignore_zero_introgression=remove_samples_wo_introgression, only_first=only_first, list_of_folders=True, upsample=112/2, ref_size=nref, target_size=ntgt)
                else:
